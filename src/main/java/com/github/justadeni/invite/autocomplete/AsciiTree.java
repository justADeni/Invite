...
public class AsciiTree {
    ...
    public List<String> autocomplete(String prefix) {
        ...
        Branch[] branches = base[prefix.charAt(0)];
        if (branches == null) return results;
        ...
        for (Branch b : branches) {
            if (b.c == (prefix.length() > 1 ? prefix.charAt(1) : 0)) {
                current = b;
                break;
            }
        }
        ...
        // If we’ve matched the whole prefix, gather completions
    ...
    private void collectWords(Branch node, StringBuilder path, List<String> results) {
        if (node == null) return;

        if (node.c != 0) path.append(node.c);
        if (node.isEnd) results.add(path.toString());

        if (node.sub != null) {
            for (Branch child : node.sub) {
